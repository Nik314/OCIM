...
def is_concurrent_cut_valid(relation_frames, partition_list, dfgs, clos, rel, div):
    ...
    for i in range(len(partition_list)):
        for a in partition_list[i]:
            for ot in rel[a]:
                if a in get_projected_start(relation_frames, partition_list[i])[ot] and not dfgs[ot][1].get(a,0):
                    return False
    ...
    for i in range(len(partition_list)):
        for a in partition_list[i]:
            for ot in rel[a]:
                if a in get_projected_end(relation_frames, partition_list[i])[ot] and not dfgs[ot][2].get(a,0):
                    return False

    return True


def is_exclusive_cut_valid(relation_frames, partition_list, dfgs, clos, rel, div):

    #check for the correct propagation of projected start activities (Section 3.1 , Equation 18)
    for i in range(len(partition_list)):
        for a in partition_list[i]:
            for ot in rel[a]:
                if a in get_projected_start(relation_frames, partition_list[i]) and not dfgs[ot][1].get(a,0):
                    return False
    ...
    for i in range(len(partition_list)):
        for a in partition_list[i]:
            for ot in rel[a]:
                if a in get_projected_end(relation_frames, partition_list[i]) and not dfgs[ot][2].get(a,0):
                    return False
    ...
    for (i, j) in itertools.combinations(range(len(partition_list)), 2):
        for a in partition_list[i]:
            for b in partition_list[j]:
                for ot in get_divergent_types(a,b,partition_list[i]+partition_list[j],div,rel):
                    if not dfgs[ot][0].get((a, b), 0) or not dfgs[ot][0].get((b, a), 0):
                        return False
    ...
    for (i, j) in itertools.combinations(range(len(partition_list)), 2):
        ...
        for a in partition_list[i]:
            for b in partition_list[j]:
                ...
                for ot in get_divergent_types(a,b,sum([partition_list[k] for k in range(i,j+1)],[]),div,rel):
                    if not dfgs[ot][0].get((a, b), 0) or not dfgs[ot][0].get((b, a), 0):
                        return False
...
def is_loop_cut_valid(relations, partition_list, dfgs, clos, rel, div):
    ...
    for a in partition_list[0]:
        for b in partition_list[1]:
            for ot in get_divergent_types(a,b,partition_list[0]+partition_list[1],div,rel):
                if not dfgs[ot][0].get((a,b),0) or not dfgs[ot][0].get((b,a),0):
                    return False
